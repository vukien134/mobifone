...
{
    public class PositionService : BaseDomainService<Position, string>
    {
        #region Privates
        private readonly IStringLocalizer<AccountingResource> _localizer;
        #endregion
        public PositionService(IRepository<Position, string> repository,
                IStringLocalizer<AccountingResource> localizer
            ) : base(repository)
        {
            _localizer = localizer;
        }
        public async Task<bool> IsExistCode(Position entity)
        {
        ...
        public override async Task CheckDuplicate(Position entity)
        {
            bool isExist = await IsExistCode(entity);
            if (isExist)
            {
                throw new AccountingException(ErrorCode.Get(GroupErrorCodes.Position, ErrorCode.Duplicate),
                        _localizer["Err:CodeAlreadyExist", entity.Code]);
            }
        }
        public async Task<Position> GetByCodeAsync(string code, string ordCode)
        {
            var queryable = await this.GetQueryableAsync();
        ...
        public async Task<List<Position>> GetDataReference(string orgCode, string filterValue)
        {
            var queryable = await this.GetQueryableAsync();
        ...
        public IQueryable<Position> GetQueryableQuickSearch(IQueryable<Position> queryable, string filterValue)
        {
