...
{
    public class ReceiptTransactionListAppService : AccountingAppService
    {
        #region Fields
        ...
        #endregion
        public ReceiptTransactionListAppService(
        ...
        #region Methods
        [Authorize(ReportPermissions.ReceiptTransactionListReportView)]
            ...
            if (dto.Parameters.Sort == "1")
            {
                ...
                var sumAktGroup0 = from p in incurredData
                                   ...
                                   select new
                                   {
                                       ...
                                       //GroupName = gr.Max(p => p.ProductName),
            ...
            if (dto.Parameters.Sort == "7")
            {
                ...
                var incurredDatas = (from p in sumAktGroup0
                                     select new IssueTransactionListDto
                                     {
                                         ...
                                         GroupCode = p.GroupCode,
                                         OrgCode = null,
                                         Note = p.GroupCode == null ? "Không có chứng từ sản phầm " : p.GroupCode + " " + p.GroupName,
                                         Amount = p.Amount,
                                         AmountCur = p.AmountCur,
        ...
        #endregion
        #region Private
        [Authorize(ReportPermissions.ReceiptTransactionListReportPrint)]
        ...
        private async Task<List<IssueTransactionListDto>> GetIncurredData(Dictionary<string, object> dic)
        {
            ...
            var incurredData = warehouseBook.Select(p => new IssueTransactionListDto()
            {
                ...
                InvoiceSymbol = p.InvoiceSymbol,
                InvoiceNumber = p.InvoiceNumber,
                Price = string.IsNullOrEmpty(p.Price.ToString()) == true ? p.ImportAmount / p.Quantity : p.Price,
                PartnerCode = string.IsNullOrEmpty(p.PartnerCode) == true ? p.PartnerCode0 : p.PartnerCode,
                PartnerName = null,
                UnitCode = p.UnitCode,
                CaseCode = p.CaseCode,
                CaseName = null,
