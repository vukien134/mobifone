...
{
    public class ExportSummaryReportAppService : AccountingAppService
    {
        #region Fields
        ...
        #endregion
        public ExportSummaryReportAppService(ReportDataService reportDataService,
        ...
        #region Methods
        [Authorize(ReportPermissions.ExportSummaryReportView)]
        public async Task<ReportResponseDto<ExportSummaryReportDto>> CreateDataAsync(ReportRequestDto<ReportBaseParameterDto> dto)
        {
            string orgCode = _webHelper.GetCurrentOrgUnit();
            var lstVoucherType = await _accountingCacheManager.GetVoucherTypeAsync();
            var voucherType = lstVoucherType.Where(p => p.Code == "PXH").FirstOrDefault();
            ...
            //var voucherCode2 = lstVoucherType.Where(p => p.Code == "PNH").FirstOrDefault();
            ...
            var dataDelete = incurredData.Where(x=>x.ProductCode == null).FirstOrDefault();
            var reusulGroups = from a in incurredData
                               ...
                               select new ExportSummaryReportDto
                               {
            ...
            incurredData = (from a in incurredData
                            ...
                            select new ExportSummaryReportDto
                            {
                                ...
                                UnitCode = pr != null? pr.UnitCode : null,
            ...
            var reportResponse = new ReportResponseDto<ExportSummaryReportDto>();
        ...
        #endregion
        #region Private
        [Authorize(ReportPermissions.ExportSummaryReportPrint)]
        ...
        private async Task<List<ExportSummaryReportDto>> GetIncurredData(Dictionary<string, object> dic)
        {

            var warehouseBook = await GetWarehouseBook(dic);
            var incurredData = warehouseBook.GroupBy(p => new { p.OrgCode, p.ProductCode }).Select(p => new ExportSummaryReportDto()
            {
                Tag = 0,
                Bold = "K",
                Sort = 2,
