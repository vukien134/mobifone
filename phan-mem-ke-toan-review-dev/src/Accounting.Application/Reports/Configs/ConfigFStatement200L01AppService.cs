...
{
    public class ConfigFStatement200L01AppService : AccountingAppService
    {
        #region Fields
        ...
        #endregion
        #region Ctor
        public ConfigFStatement200L01AppService(
        ...
        #endregion
        #region Methods
        public async Task<List<FStatement200L01Dto>> ListAsync()
        {
            var yearCategory = await this.GetYearCategoryAsync();
            var lstTenant = (await _fStatement200L01Service.GetQueryableAsync()).Where(p => p.OrgCode == yearCategory.OrgCode && 
                            p.Year == yearCategory.Year && p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstTenant.Count > 0)
            {
                return lstTenant.Select(p => ObjectMapper.Map<TenantFStatement200L01, FStatement200L01Dto>(p))
                        .OrderBy(p => p.Ord).ToList();
            }
            var lstDefault = (await _defaultFStatement200L01Service.GetQueryableAsync()).Where(p => p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstDefault.Count > 0)
            {
                return lstDefault.Select(p => ObjectMapper.Map<DefaultFStatement200L01, FStatement200L01Dto>(p))
                            .OrderBy(p => p.Ord).ToList();
            }
            return new List<FStatement200L01Dto>();
        }
        public async Task Save(ListRequestDto<CrudFStatement200L01Dto> dtos)
        {
            var yearCategory = await this.GetYearCategoryAsync();
            try
            {
                ...
                await _fStatement200L01Service.DeleteManyAsync(lstTenant);
                await CreateTenantFStatement200L01Async(dtos.Data,yearCategory);
        ...
        private async Task CreateTenantFStatement200L01Async(List<CrudFStatement200L01Dto> dtos,YearCategory yearCategory)
        {
            ...
            var entities = dtos.Select(p => ObjectMapper.Map<CrudFStatement200L01Dto, TenantFStatement200L01>(p))
                            .ToList();
            await _fStatement200L01Service.CreateManyAsync(entities);
        }
        #endregion
    }
}
