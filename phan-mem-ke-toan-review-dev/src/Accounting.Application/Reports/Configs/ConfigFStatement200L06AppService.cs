...
{
    public class ConfigFStatement200L06AppService : AccountingAppService
    {
        #region Fields
        ...
        #endregion
        #region Ctor
        public ConfigFStatement200L06AppService(
        ...
        #endregion
        #region Methods
        public async Task<List<FStatement200L06Dto>> ListAsync()
        {
            var yearCategory = await this.GetYearCategoryAsync();
            var lstTenant = (await _fStatement200L06Service.GetQueryableAsync()).Where(p => p.OrgCode == yearCategory.OrgCode && 
                            p.Year == yearCategory.Year && p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstTenant.Count > 0)
            {
                return lstTenant.Select(p => ObjectMapper.Map<TenantFStatement200L06, FStatement200L06Dto>(p))
                        .OrderBy(p => p.Ord).ToList();
            }
            var lstDefault = (await _defaultFStatement200L06Service.GetQueryableAsync()).Where(p => p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstDefault.Count > 0)
            {
                return lstDefault.Select(p => ObjectMapper.Map<DefaultFStatement200L06, FStatement200L06Dto>(p))
                            .OrderBy(p => p.Ord).ToList();
            }
            return new List<FStatement200L06Dto>();
        }
        public async Task Save(ListRequestDto<CrudFStatement200L06Dto> dtos)
        {
            var yearCategory = await this.GetYearCategoryAsync();
            try
            {
                ...
                await _fStatement200L06Service.DeleteManyAsync(lstTenant);
                await CreateTenantFStatement200L06Async(dtos.Data,yearCategory);
        ...
        private async Task CreateTenantFStatement200L06Async(List<CrudFStatement200L06Dto> dtos,YearCategory yearCategory)
        {
            ...
            var entities = dtos.Select(p => ObjectMapper.Map<CrudFStatement200L06Dto, TenantFStatement200L06>(p))
                            .ToList();
            await _fStatement200L06Service.CreateManyAsync(entities);
        }
        #endregion
    }
}
