...
{
    public class ConfigFStatement200L15AppService : AccountingAppService
    {
        #region Fields
        ...
        #endregion
        #region Ctor
        public ConfigFStatement200L15AppService(
        ...
        #endregion
        #region Methods
        public async Task<List<FStatement200L15Dto>> ListAsync()
        {
            var yearCategory = await this.GetYearCategoryAsync();
            var lstTenant = (await _fStatement200L15Service.GetQueryableAsync()).Where(p => p.OrgCode == yearCategory.OrgCode && 
                            p.Year == yearCategory.Year && p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstTenant.Count > 0)
            {
                return lstTenant.Select(p => ObjectMapper.Map<TenantFStatement200L15, FStatement200L15Dto>(p))
                        .OrderBy(p => p.Ord).ToList();
            }
            var lstDefault = (await _defaultFStatement200L15Service.GetQueryableAsync()).Where(p => p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstDefault.Count > 0)
            {
                return lstDefault.Select(p => ObjectMapper.Map<DefaultFStatement200L15, FStatement200L15Dto>(p))
                            .OrderBy(p => p.Ord).ToList();
            }
            return new List<FStatement200L15Dto>();
        }
        public async Task Save(ListRequestDto<CrudFStatement200L15Dto> dtos)
        {
            var yearCategory = await this.GetYearCategoryAsync();
            try
            {
                ...
                await _fStatement200L15Service.DeleteManyAsync(lstTenant);
                await CreateTenantFStatement200L15Async(dtos.Data,yearCategory);
        ...
        private async Task CreateTenantFStatement200L15Async(List<CrudFStatement200L15Dto> dtos,YearCategory yearCategory)
        {
            ...
            var entities = dtos.Select(p => ObjectMapper.Map<CrudFStatement200L15Dto, TenantFStatement200L15>(p))
                            .ToList();
            await _fStatement200L15Service.CreateManyAsync(entities);
        }
        #endregion
    }
}
