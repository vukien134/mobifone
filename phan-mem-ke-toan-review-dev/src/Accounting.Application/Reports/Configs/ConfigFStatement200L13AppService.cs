...
{
    public class ConfigFStatement200L13AppService : AccountingAppService
    {
        #region Fields
        ...
        #endregion
        #region Ctor
        public ConfigFStatement200L13AppService(
        ...
        #endregion
        #region Methods
        public async Task<List<FStatement200L13Dto>> ListAsync()
        {
            var yearCategory = await this.GetYearCategoryAsync();
            var lstTenant = (await _fStatement200L13Service.GetQueryableAsync()).Where(p => p.OrgCode == yearCategory.OrgCode && 
                            p.Year == yearCategory.Year && p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstTenant.Count > 0)
            {
                return lstTenant.Select(p => ObjectMapper.Map<TenantFStatement200L13, FStatement200L13Dto>(p))
                        .OrderBy(p => p.Ord).ToList();
            }
            var lstDefault = (await _defaultFStatement200L13Service.GetQueryableAsync()).Where(p => p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstDefault.Count > 0)
            {
                return lstDefault.Select(p => ObjectMapper.Map<DefaultFStatement200L13, FStatement200L13Dto>(p))
                            .OrderBy(p => p.Ord).ToList();
            }
            return new List<FStatement200L13Dto>();
        }
        public async Task Save(ListRequestDto<CrudFStatement200L13Dto> dtos)
        {
            var yearCategory = await this.GetYearCategoryAsync();
            try
            {
                ...
                await _fStatement200L13Service.DeleteManyAsync(lstTenant);
                await CreateTenantFStatement200L13Async(dtos.Data,yearCategory);
        ...
        private async Task CreateTenantFStatement200L13Async(List<CrudFStatement200L13Dto> dtos,YearCategory yearCategory)
        {
            ...
            var entities = dtos.Select(p => ObjectMapper.Map<CrudFStatement200L13Dto, TenantFStatement200L13>(p))
                            .ToList();
            await _fStatement200L13Service.CreateManyAsync(entities);
        }
        #endregion
    }
}
