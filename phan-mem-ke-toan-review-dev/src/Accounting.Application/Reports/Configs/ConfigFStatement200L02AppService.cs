...
{
    public class ConfigFStatement200L02AppService : AccountingAppService
    {
        #region Fields
        ...
        #endregion
        #region Ctor
        public ConfigFStatement200L02AppService(
        ...
        #endregion
        #region Methods
        public async Task<List<FStatement200L02Dto>> ListAsync()
        {
            var yearCategory = await this.GetYearCategoryAsync();
            var lstTenant = (await _fStatement200L02Service.GetQueryableAsync()).Where(p => p.OrgCode == yearCategory.OrgCode && 
                            p.Year == yearCategory.Year && p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstTenant.Count > 0)
            {
                return lstTenant.Select(p => ObjectMapper.Map<TenantFStatement200L02, FStatement200L02Dto>(p))
                        .OrderBy(p => p.Ord).ToList();
            }
            var lstDefault = (await _defaultFStatement200L02Service.GetQueryableAsync()).Where(p => p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstDefault.Count > 0)
            {
                return lstDefault.Select(p => ObjectMapper.Map<DefaultFStatement200L02, FStatement200L02Dto>(p))
                            .OrderBy(p => p.Ord).ToList();
            }
            return new List<FStatement200L02Dto>();
        }
        public async Task Save(ListRequestDto<CrudFStatement200L02Dto> dtos)
        {
            var yearCategory = await this.GetYearCategoryAsync();
            try
            {
                ...
                await _fStatement200L02Service.DeleteManyAsync(lstTenant);
                await CreateTenantFStatement200L02Async(dtos.Data,yearCategory);
        ...
        private async Task CreateTenantFStatement200L02Async(List<CrudFStatement200L02Dto> dtos,YearCategory yearCategory)
        {
            ...
            var entities = dtos.Select(p => ObjectMapper.Map<CrudFStatement200L02Dto, TenantFStatement200L02>(p))
                            .ToList();
            await _fStatement200L02Service.CreateManyAsync(entities);
        }
        #endregion
    }
}
