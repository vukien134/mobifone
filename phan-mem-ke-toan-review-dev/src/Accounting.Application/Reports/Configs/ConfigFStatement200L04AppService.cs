...
{
    public class ConfigFStatement200L04AppService : AccountingAppService
    {
        #region Fields
        ...
        #endregion
        #region Ctor
        public ConfigFStatement200L04AppService(
        ...
        #endregion
        #region Methods
        public async Task<List<FStatement200L04Dto>> ListAsync()
        {
            var yearCategory = await this.GetYearCategoryAsync();
            var lstTenant = (await _fStatement200L04Service.GetQueryableAsync()).Where(p => p.OrgCode == yearCategory.OrgCode && 
                            p.Year == yearCategory.Year && p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstTenant.Count > 0)
            {
                return lstTenant.Select(p => ObjectMapper.Map<TenantFStatement200L04, FStatement200L04Dto>(p))
                        .OrderBy(p => p.Ord).ToList();
            }
            var lstDefault = (await _defaultFStatement200L04Service.GetQueryableAsync()).Where(p => p.UsingDecision == yearCategory.UsingDecision.Value).ToList();
            if (lstDefault.Count > 0)
            {
                return lstDefault.Select(p => ObjectMapper.Map<DefaultFStatement200L04, FStatement200L04Dto>(p))
                            .OrderBy(p => p.Ord).ToList();
            }
            return new List<FStatement200L04Dto>();
        }
        public async Task Save(ListRequestDto<CrudFStatement200L04Dto> dtos)
        {
            var yearCategory = await this.GetYearCategoryAsync();
            try
            {
                ...
                await _fStatement200L04Service.DeleteManyAsync(lstTenant);
                await CreateTenantFStatement200L04Async(dtos.Data,yearCategory);
        ...
        private async Task CreateTenantFStatement200L04Async(List<CrudFStatement200L04Dto> dtos,YearCategory yearCategory)
        {
            ...
            var entities = dtos.Select(p => ObjectMapper.Map<CrudFStatement200L04Dto, TenantFStatement200L04>(p))
                            .ToList();
            await _fStatement200L04Service.CreateManyAsync(entities);
        }
        #endregion
    }
}
